...
% sp_l2error = zeros(ITER, length(sep)); sp_suppdist = zeros(ITER, length(sep));
% sp_orig_l2error = zeros(ITER, length(sep)); sp_orig_suppdist = zeros(ITER, length(sep));
% lsomp_l2error = zeros(ITER, length(sep)); lsomp_suppdist = zeros(ITER, length(sep));
seqsbl_l2error = zeros(ITER, length(sep)); seqsbl_suppdist = zeros(ITER, length(sep)); seqsbl_timecomp = zeros(ITER, length(sep)); seqsbl_kpset=zeros(ITER,length(sep),suppsizelist);
seqsbl_Cinv_l2error = zeros(ITER, length(sep)); seqsbl_Cinv_suppdist = zeros(ITER, length(sep)); seqsbl_Cinv_timecomp = zeros(ITER, length(sep));
...
seqsbl_targetsj_l2error = zeros(ITER, length(sep)); seqsbl_targetsj_suppdist = zeros(ITER, length(sep)); seqsbl_targetsj_timecomp = zeros(ITER, length(sep));
seqsbl_ximprov_l2error = zeros(ITER, length(sep)); seqsbl_ximprov_suppdist = zeros(ITER, length(sep)); seqsbl_ximprov_timecomp = zeros(ITER, length(sep));
...
omp_l2error = zeros(ITER, length(sep)); omp_suppdist = zeros(ITER, length(sep)); omp_timecomp = zeros(ITER, length(sep));omp_kpset=zeros(ITER,length(sep),suppsizelist);
% mp_l2error = zeros(ITER, length(sep)); mp_suppdist = zeros(ITER, length(sep));
% weakmp_l2error = zeros(ITER, length(sep)); weakmp_suppdist = zeros(ITER, length(sep));
% thresh_l2error = zeros(ITER, length(sep)); thresh_suppdist = zeros(ITER, length(sep));
% L1_l2error = zeros(ITER, length(sep)); L1_suppdist = zeros(ITER, length(sep));
% RL1_l2error = zeros(ITER, length(sep)); RL1_suppdist = zeros(ITER, length(sep));
% reg_IRLS_l2error = zeros(ITER, length(sep)); reg_IRLS_suppdist = zeros(ITER, length(sep));
% SBL_l2error = zeros(ITER, length(sep)); SBL_suppdist = zeros(ITER, length(sep));
...
%                     end
                ...
                seqsbl_kpset(iter,sep_iter,:)=reshape(kpset,1,1,length(kpset));%reshape(sort(kpset),1,1,length(kpset));
%                 xhat(kpset(1:p))=gamma_est(kpset(1:p)).*qj(kpset(1:p));
                ...
                seqsbl_timecomp(iter,sep_iter)=toc;
...
%                 %             profile off
%                 %             xhat(kpset(1:p))=diag(gamma_est(kpset(1:p)))*A(:,kpset(1:p))'*((A*diag(gamma_est)*A'+sigma_n*eye(m))\y);%pinv(A(:,kpset(1:p)))*y;
%                 seqsbl_Cinv_timecomp(iter,sep_iter)=toc;
% %                 seqsbl_Cinv_l2error(iter,sep_iter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
%                 seqsbl_Cinv_suppdist(iter,sep_iter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
...
%                 %             profile off
%                 %             xhat(kpset(1:p))=diag(gamma_est(kpset(1:p)))*A(:,kpset(1:p))'*((A*diag(gamma_est)*A'+sigma_n*eye(m))\y);%pinv(A(:,kpset(1:p)))*y;
%                 seqsbl_nvarNotgivenwvarby10_timecomp(iter,sep_iter)=toc;
...
%                 %             profile off
%                 %             xhat(kpset(1:p))=diag(gamma_est(kpset(1:p)))*A(:,kpset(1:p))'*((A*diag(gamma_est)*A'+sigma_n*eye(m))\y);%pinv(A(:,kpset(1:p)))*y;
%                 seqsbl_nvarNotgivenwvar10_timecomp(iter,sep_iter)=toc;
...
%                 seqsbl_nvarNotgivenwvar10_suppdist(iter,sep_iter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                %             xhat(kpset(1:p))=gamma_est(kpset(1:p)).*qj(kpset(1:p));
                %             %             xhat(kpset(1:p))=diag(gamma_est(kpset(1:p)))*A(:,kpset(1:p))'*((A*diag(gamma_est)*A'+sigma_n*eye(m))\y);%pinv(A(:,kpset(1:p)))*y;
                %             seqsbl_targetsj_timecomp(iter,sep_iter)=toc;
                %             %             profile off
                %             seqsbl_targetsj_l2error(iter,sep_iter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %             seqsbl_targetsj_suppdist(iter,sep_iter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                % %             xhat(kpset(1:p))=diag(gamma_est(kpset(1:p)))*A(:,kpset(1:p))'*((A*diag(gamma_est)*A'+sigma_n*eye(m))\y);%pinv(A(:,kpset(1:p)))*y;
                %             seqsbl_ximprov_timecomp(iter,sep_iter)=toc;
                %             seqsbl_ximprov_l2error(iter,sep_iter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %             seqsbl_ximprov_suppdist(iter,sep_iter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                omp_kpset(iter,sep_iter,:)=reshape(kpset,1,1,length(kpset));%reshape(sort(kpset),1,1,length(kpset));
%                 xhat(kpset) = pinv(Atest(:,kpset))*y;
                omp_timecomp(iter,sep_iter)=toc;
...
%                 redcompomp_timecomp(iter,sep_iter)=toc;
...
%                 redcompomp_suppdist(iter,sep_iter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                %         lsomp_l2error(iter,sep_iter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %         lsomp_suppdist(iter,sep_iter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                %         xhat = zeros(m,1);
                %         xhat(kpset) = pinv(A(:,kpset))*y;
                %         mp_l2error(iter,sep_iter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %         mp_suppdist(iter,sep_iter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                %         xhat = zeros(m,1);
                %         xhat(kpset) = pinv(A(:,kpset))*y;
                %         weakmp_l2error(iter,sep_iter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %         weakmp_suppdist(iter,sep_iter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                %         xhat = zeros(n,1);
                %         xhat(kpset) = pinv(A(:,kpset))*y;
                %         sp_l2error(iter,sep_iter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %         sp_suppdist(iter,sep_iter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                
                %% Subspace Pursuit Original Code
                %         s=suppsizelist(isuppsize);
                %         Rec = CSRec_SP(s,A,y);
                %         xhat = Rec.x_hat; [~,kpset] = maxk(abs(xhat),s);
                %         sp_orig_l2error(iter,sep_iter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %         sp_orig_suppdist(iter,sep_iter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                %         xhat = xp;
                %         kpset = ind;
                %         thresh_l2error(iter,sep_iter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %         thresh_suppdist(iter,sep_iter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                %         L1_l2error(iter,sep_iter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %         L1_suppdist(iter,sep_iter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                %         end
                %         kpset = find(abs(xhat)>1e-4);
                %         RL1_l2error(iter,sep_iter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %         RL1_suppdist(iter,sep_iter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                %         reg_IRLS_l2error(iter,sep_iter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %         reg_IRLS_suppdist(iter,sep_iter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                %         SBL_l2error(iter,sep_iter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %         SBL_suppdist(iter,sep_iter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
...
figure
% support size=1
sbl_supp_succ_rate=sum(seqsbl_suppdist==0,1);%seqsbl_Cinv_suppdist
sbl_supp_succ_prob=sbl_supp_succ_rate/ITER;%meas. size X supp. size
plot(sep,sbl_supp_succ_prob, '-sb', 'LineWidth', 2, 'MarkerSize', 15)
hold on
omp_supp_succ_rate=sum(omp_suppdist==0,1);%seqsbl_Cinv_suppdist
omp_supp_succ_prob=omp_supp_succ_rate/ITER;%meas. size X supp. size
plot(sep,omp_supp_succ_prob, '-or', 'LineWidth', 2, 'MarkerSize', 8)
grid on
...
% Recovered Support Histogram for source separation=1 grid point
figure
subplot(221)
histogram(seqsbl_kpset(:,1,1),80.5:120.5)
hold on
plot(permute_n(1)*ones(1,501), 0:500,'--k','LineWidth',2)
plot(permute_n(2)*ones(1,501), 0:500,'--k','LineWidth',2)
grid on
axis([90 115 0 500])
yticks(0:100:500)
xticks(80:5:120)
...
xlabel('Grid point recovered for Source 1')
ylabel('Realizations count')
legend('Histogram', 'Source 1', 'Source 2')

% figure
subplot(222)
histogram(seqsbl_kpset(:,1,2),80.5:120.5)
hold on
plot(permute_n(1)*ones(1,501), 0:500,'--k','LineWidth',2)
plot(permute_n(2)*ones(1,501), 0:500,'--k','LineWidth',2)
grid on
...
ax=gca; ax.GridAlpha=0.5;
set(gca, 'FontWeight', 'Bold', 'FontSize', 14)
xlabel('Grid point recovered for Source 2')
ylabel('Realizations count')
legend('Histogram', 'Source 1', 'Source 2')

% figure
subplot(223)
histogram(omp_kpset(:,1,1),80.5:120.5)
hold on
plot(permute_n(1)*ones(1,501), 0:500,'--k','LineWidth',2)
plot(permute_n(2)*ones(1,501), 0:500,'--k','LineWidth',2)
grid on
...
% figure
subplot(224)
histogram(omp_kpset(:,1,2),80.5:120.5)
hold on
plot(permute_n(1)*ones(1,501), 0:500,'--k','LineWidth',2)
plot(permute_n(2)*ones(1,501), 0:500,'--k','LineWidth',2)
grid on
...
% plot(sep,sbl_supp_succ_prob(:,isuppsize), '-sb', 'LineWidth', 2, 'MarkerSize', 15)
% plot(sep,omp_supp_succ_prob([1 5:end],isuppsize), '-or', 'LineWidth', 2, 'MarkerSize', 8)
% isuppsize=5; disp(['Support size=' num2str(suppsizelist(isuppsize))])
% plot(sep,sbl_supp_succ_prob(:,isuppsize), '-sb', 'LineWidth', 2, 'MarkerSize', 15)
% plot(sep,omp_supp_succ_prob(:,isuppsize), '-or', 'LineWidth', 2, 'MarkerSize', 8)
% legend('LWS-SBL', 'OMP','Location', 'SouthEast')
%%
