...
n = 200;%256;
suppsizelist = 2;%[1 5:5:20];
datagen = 0; % 0: complex (for array signal processing), 2: real (for Gaussian random measurement matrices)
% epsilon = 1e-2;
% r = Inf; %suppsize;
ITER = 500;
SNR=30; % in dB
...
sep=1:50; % make sure (max. sep X 2 X suppsizemax)<=n to ensure sep is always possible
...
% Creating index matrix to 
[x_ind,y_ind]=meshgrid(0:n-1,0:n-1);
gridptdiff_mat1=abs(x_ind-y_ind);
gridptdiff_mat2=-gridptdiff_mat1+n;
gridptdiff_mat=min(gridptdiff_mat1,gridptdiff_mat2);
for iter_mc=1:n-1
    AtA_tmp(gridptdiff_mat==(iter_mc-1))=0;
    mc(iter_mc) = max(max(AtA_tmp));
end

% sp_l2error = zeros(ITER, length(sep), length(m)); sp_suppdist = zeros(ITER, length(sep), length(m));
% sp_orig_l2error = zeros(ITER, length(sep), length(m)); sp_orig_suppdist = zeros(ITER, length(sep), length(m));
% lsomp_l2error = zeros(ITER, length(sep), length(m)); lsomp_suppdist = zeros(ITER, length(sep), length(m));
seqsbl_l2error = zeros(ITER, length(sep), length(m)); seqsbl_suppdist = zeros(ITER, length(sep), length(m)); seqsbl_timecomp = zeros(ITER, length(sep), length(m)); seqsbl_kpset=zeros(ITER,length(sep),suppsizelist);
seqsbl_Cinv_l2error = zeros(ITER, length(sep), length(m)); seqsbl_Cinv_suppdist = zeros(ITER, length(sep), length(m)); seqsbl_Cinv_timecomp = zeros(ITER, length(sep), length(m));
seqsbl_nvarNotgivenwvarby10_l2error = zeros(ITER, length(sep), length(m)); seqsbl_nvarNotgivenwvarby10_suppdist = zeros(ITER, length(sep), length(m)); seqsbl_nvarNotgivenwvarby10_timecomp = zeros(ITER, length(sep), length(m));
seqsbl_nvarNotgivenwvar10_l2error = zeros(ITER, length(sep), length(m)); seqsbl_nvarNotgivenwvar10_suppdist = zeros(ITER, length(sep), length(m)); seqsbl_nvarNotgivenwvar10_timecomp = zeros(ITER, length(sep), length(m));
seqsbl_targetsj_l2error = zeros(ITER, length(sep), length(m)); seqsbl_targetsj_suppdist = zeros(ITER, length(sep), length(m)); seqsbl_targetsj_timecomp = zeros(ITER, length(sep), length(m));
seqsbl_ximprov_l2error = zeros(ITER, length(sep), length(m)); seqsbl_ximprov_suppdist = zeros(ITER, length(sep), length(m)); seqsbl_ximprov_timecomp = zeros(ITER, length(sep), length(m));
redcompomp_l2error = zeros(ITER, length(sep), length(m)); redcompomp_suppdist = zeros(ITER, length(sep), length(m)); redcompomp_timecomp = zeros(ITER, length(sep), length(m));
omp_l2error = zeros(ITER, length(sep), length(m)); omp_suppdist = zeros(ITER, length(sep), length(m)); omp_timecomp = zeros(ITER, length(sep), length(m));omp_kpset=zeros(ITER,length(sep),suppsizelist);
% mp_l2error = zeros(ITER, length(sep), length(m)); mp_suppdist = zeros(ITER, length(sep), length(m));
% weakmp_l2error = zeros(ITER, length(sep), length(m)); weakmp_suppdist = zeros(ITER, length(sep), length(m));
% thresh_l2error = zeros(ITER, length(sep), length(m)); thresh_suppdist = zeros(ITER, length(sep), length(m));
% L1_l2error = zeros(ITER, length(sep), length(m)); L1_suppdist = zeros(ITER, length(sep), length(m));
% RL1_l2error = zeros(ITER, length(sep), length(m)); RL1_suppdist = zeros(ITER, length(sep), length(m));
% reg_IRLS_l2error = zeros(ITER, length(sep), length(m)); reg_IRLS_suppdist = zeros(ITER, length(sep), length(m));
% SBL_l2error = zeros(ITER, length(sep), length(m)); SBL_suppdist = zeros(ITER, length(sep), length(m));
...
for iter=1:ITER
    ...
    permute_n=[n/2+1:n];%randsample(n, n); % without replacement
...
%                     end
                ...
                seqsbl_kpset(iter,sep_iter,:)=reshape(sort(kpset),1,1,length(kpset));
%                 xhat(kpset(1:p))=gamma_est(kpset(1:p)).*qj(kpset(1:p));
                ...
                seqsbl_timecomp(iter,sep_iter,miter)=toc;
%                 seqsbl_l2error(iter,sep_iter,miter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                seqsbl_suppdist(iter,sep_iter,miter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
...
%                 %             profile off
%                 %             xhat(kpset(1:p))=diag(gamma_est(kpset(1:p)))*A(:,kpset(1:p))'*((A*diag(gamma_est)*A'+sigma_n*eye(m))\y);%pinv(A(:,kpset(1:p)))*y;
%                 seqsbl_Cinv_timecomp(iter,sep_iter,miter)=toc;
% %                 seqsbl_Cinv_l2error(iter,sep_iter,miter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
%                 seqsbl_Cinv_suppdist(iter,sep_iter,miter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
...
%                 xhat(kpset(kpset>0))=gamma_est(kpset(kpset>0)).*qj(kpset(kpset>0));
                ...
                %             profile off
                %             xhat(kpset(1:p))=diag(gamma_est(kpset(1:p)))*A(:,kpset(1:p))'*((A*diag(gamma_est)*A'+sigma_n*eye(m))\y);%pinv(A(:,kpset(1:p)))*y;
                seqsbl_nvarNotgivenwvarby10_timecomp(iter,sep_iter,miter)=toc;
%                 seqsbl_nvarNotgivenwvarby10_l2error(iter,sep_iter,miter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                seqsbl_nvarNotgivenwvarby10_suppdist(iter,sep_iter,miter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
...
%                 xhat(kpset(kpset>0))=gamma_est(kpset(kpset>0)).*qj(kpset(kpset>0));
                ...
                %             profile off
                %             xhat(kpset(1:p))=diag(gamma_est(kpset(1:p)))*A(:,kpset(1:p))'*((A*diag(gamma_est)*A'+sigma_n*eye(m))\y);%pinv(A(:,kpset(1:p)))*y;
                seqsbl_nvarNotgivenwvar10_timecomp(iter,sep_iter,miter)=toc;
%                 seqsbl_nvarNotgivenwvar10_l2error(iter,sep_iter,miter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                seqsbl_nvarNotgivenwvar10_suppdist(iter,sep_iter,miter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                %             xhat(kpset(1:p))=gamma_est(kpset(1:p)).*qj(kpset(1:p));
                %             %             xhat(kpset(1:p))=diag(gamma_est(kpset(1:p)))*A(:,kpset(1:p))'*((A*diag(gamma_est)*A'+sigma_n*eye(m))\y);%pinv(A(:,kpset(1:p)))*y;
                %             seqsbl_targetsj_timecomp(iter,sep_iter,miter)=toc;
                %             %             profile off
                %             seqsbl_targetsj_l2error(iter,sep_iter,miter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %             seqsbl_targetsj_suppdist(iter,sep_iter,miter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                % %             xhat(kpset(1:p))=diag(gamma_est(kpset(1:p)))*A(:,kpset(1:p))'*((A*diag(gamma_est)*A'+sigma_n*eye(m))\y);%pinv(A(:,kpset(1:p)))*y;
                %             seqsbl_ximprov_timecomp(iter,sep_iter,miter)=toc;
                %             seqsbl_ximprov_l2error(iter,sep_iter,miter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %             seqsbl_ximprov_suppdist(iter,sep_iter,miter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                omp_kpset(iter,sep_iter,:)=reshape(sort(kpset),1,1,length(kpset));
%                 xhat(kpset) = pinv(Atest(:,kpset))*y;
                omp_timecomp(iter,sep_iter,miter)=toc;
%                 omp_l2error(iter,sep_iter,miter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                omp_suppdist(iter,sep_iter,miter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
...
%                 xhat(kpset) = linsolve(Rmat,ynorm,opts);
                redcompomp_timecomp(iter,sep_iter,miter)=toc;
%                 redcompomp_l2error(iter,sep_iter,miter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                redcompomp_suppdist(iter,sep_iter,miter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                %         lsomp_l2error(iter,sep_iter,miter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %         lsomp_suppdist(iter,sep_iter,miter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                %         xhat = zeros(m,1);
                %         xhat(kpset) = pinv(A(:,kpset))*y;
                %         mp_l2error(iter,sep_iter,miter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %         mp_suppdist(iter,sep_iter,miter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                %         xhat = zeros(m,1);
                %         xhat(kpset) = pinv(A(:,kpset))*y;
                %         weakmp_l2error(iter,sep_iter,miter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %         weakmp_suppdist(iter,sep_iter,miter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                %         xhat = zeros(n,1);
                %         xhat(kpset) = pinv(A(:,kpset))*y;
                %         sp_l2error(iter,sep_iter,miter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %         sp_suppdist(iter,sep_iter,miter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                
                %% Subspace Pursuit Original Code
                %         s=suppsizelist(isuppsize);
                %         Rec = CSRec_SP(s,A,y);
                %         xhat = Rec.x_hat; [~,kpset] = maxk(abs(xhat),s);
                %         sp_orig_l2error(iter,sep_iter,miter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %         sp_orig_suppdist(iter,sep_iter,miter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                %         xhat = xp;
                %         kpset = ind;
                %         thresh_l2error(iter,sep_iter,miter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %         thresh_suppdist(iter,sep_iter,miter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                %         L1_l2error(iter,sep_iter,miter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %         L1_suppdist(iter,sep_iter,miter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                %         end
                %         kpset = find(abs(xhat)>1e-4);
                %         RL1_l2error(iter,sep_iter,miter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %         RL1_suppdist(iter,sep_iter,miter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                %         reg_IRLS_l2error(iter,sep_iter,miter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %         reg_IRLS_suppdist(iter,sep_iter,miter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
                ...
                %         SBL_l2error(iter,sep_iter,miter) = vecnorm(x-xhat)^2/vecnorm(x)^2;
                %         SBL_suppdist(iter,sep_iter,miter) = 1-length(intersect(supp,kpset))/max(suppsize,length(kpset));
...
imagesc(sep,m,sbl_supp_succ_prob)
yticks([1 5 10:10:100]); xticks(sep)
colormap('gray'); colorbar
set(gca,'YDir','normal');
xlabel(['Separation in grid points (1 pt.=' num2str(2/n) ' sep. in u-space)'])
ylabel('Measurement size (m)')
set(gca, 'FontWeight', 'bold', 'FontSize', 12)
axis([0.5 15.5 0.5 103])
grid on
...
imagesc(sep,m,omp_supp_succ_prob)
yticks([1 5 10:10:100]); xticks(sep)
colormap('gray'); colorbar
set(gca,'YDir','normal');
xlabel(['Separation in grid points (1 pt.=' num2str(2/n) ' sep. in u-space)'])
